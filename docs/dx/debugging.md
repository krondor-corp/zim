...
Each dev node keeps its vault log at `./data/<nick>/log.sqlite`; the hub's app state (users, devices, escrow) is at `./data/zim-hub/state/hub.db`.

```bash
sqlite3 data/alice/log.sqlite '.tables'
sqlite3 data/alice/log.sqlite 'SELECT * FROM vault_log ORDER BY height DESC LIMIT 10;'
sqlite3 data/zim-hub/state/hub.db '.tables'
```
