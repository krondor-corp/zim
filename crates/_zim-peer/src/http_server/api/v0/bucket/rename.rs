...
pub async fn handler(
    ...
    let abs_old_path = AbsPath::from_abs(old_path);
    let abs_new_path = AbsPath::from_abs(new_path);
    ...
    let _source_node = mount.get(&abs_old_path).await.map_err(|_| {
        RenameError::SourceNotFound(format!("Source path not found: {}", req.old_path))
    })?;

    // Check if destination already exists
    if mount.get(&abs_new_path).await.is_ok() {
    ...
    let file_data = mount.cat(&abs_old_path).await.map_err(|e| {
    ...
    mount.rm(&abs_old_path).await.map_err(|e| {
    ...
    mount.add(&abs_new_path, reader).await.map_err(|e| {
