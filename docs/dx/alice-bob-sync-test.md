...
./bin/dev cli alice vault create demo
echo "from alice" | ./bin/dev cli alice vault add demo /readme.md

# 6. Alice shares with bob — the OfferShare effect bootstraps bob
./bin/dev cli alice vault shares add demo bob
sleep 12   # DHT discovery + OfferShare → bob's vault open

# 7. Bob can see + read alice's file
./bin/dev cli bob vault list
./bin/dev cli bob vault ls demo /
./bin/dev cli bob vault cat demo /readme.md
# expect: "from alice"

# 8. Bob writes — announce_head fires → alice pulls
echo "from bob" | ./bin/dev cli bob vault add demo /from_bob.txt
sleep 5

# 9. Alice has bob's file
./bin/dev cli alice vault ls demo /
./bin/dev cli alice vault cat demo /from_bob.txt
# expect: "from bob"

# 10. Both heights advanced past 1 (the cache-bug regression)
./bin/dev cli alice vault head demo
./bin/dev cli bob   vault head demo
...
|---|---|
| `bob vault list` after share | shows `demo` |
| `bob vault cat demo /readme.md` | prints `from alice` |
| `alice vault cat demo /from_bob.txt` | prints `from bob` |
