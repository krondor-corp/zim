...
use super::cache::{CachedAttr, CachedContent, CachedDirEntry, FileCache};
...
struct WriteBuffer {
    /// Inode this handle writes to — lets `setattr` truncation find and
    /// resize buffers that were seeded before the truncate arrived
    /// (macFUSE opens with plain O_WRONLY, then truncates via setattr).
    ino: u64,
    data: Vec<u8>,
    dirty: bool,
}
...
impl<B: BlobStore, L: VaultLog> Filesystem for FuseFs<B, L> {
    fn init(
        ...
        tracing::info!("fuse mount initialised");
        Ok(())
    }

    fn destroy(&mut self) {
        tracing::info!("fuse mount destroyed");
    }
    ...
    fn setattr(
        ...
        if let Some(size) = size {
            ...
            // Open write buffers were seeded from pre-truncate content;
            // resize them too or their flush resurrects the old tail.
            let mut buffers = self.write_buffers.lock().unwrap();
            for buf in buffers.values_mut().filter(|b| b.ino == ino) {
                if buf.data.len() != size as usize {
    ...
    fn open(&mut self, _req: &Request<'_>, ino: u64, flags: i32, reply: ReplyOpen) {
        ...
        if write {
            // O_TRUNC starts from an empty file — and must commit the
            // truncation even if nothing is written (`> file`), so the
            // buffer starts dirty. Otherwise seed with current content
            // so partial writes modify the right base (whole-file
            // replace on flush).
