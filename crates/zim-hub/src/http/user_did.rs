...
//!
//! Per the did:web spec, `did:web:<host>:u:<user_id>` resolves to
//! `https://<host>/u/<user_id>/did.json`. The document lists **one
//! verification method per enrolled key** (web key + daemons), straight from
...
struct DidDocument {
    ...
    /// Every key may authenticate as this identity.
...
fn key_multibase(pk: &PublicKey) -> String {
    let key_did = zim_did::Identity::Key(*pk).to_did().to_string();
    key_did
