...
//!
//! Hub verifier lives at `zim_hub::http::auth::jwt`. This module is the
//! client side: mint a fresh, short-lived JWT for each request.
//!
...
//! ```
//!
//! No long-lived bearer token is held — the only secret is the
//! `identity.key` the daemon enrolled at device-code time.
...
/// window tight without forcing precise clock sync (the hub tolerates
/// ±30s skew, leaving ~30s of usable validity at worst-case drift).
const TTL_SECS: i64 = 60;

/// Mint a JWT for `hub_url`, signed by `secret`.
///
/// `hub_url` is normalized (trailing slash stripped) so callers can pass
/// `hub-session.json`'s `hub_url` field through verbatim without a
/// trailing-slash mismatch against the hub's configured host.
