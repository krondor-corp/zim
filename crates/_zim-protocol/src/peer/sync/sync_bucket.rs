    ...
    L::Error: std::error::Error + Send + Sync + 'static,
{
    ...
    for (manifest, link) in manifests.iter() {
        ...
        let height = manifest.height();
        // T-008: whole-bucket publish state is gone; per-file/folder
        // publication lives in `manifest.published_set()`. The log's
        // `is_published` column is `false` for any new sync.
        ...
        if should_sync {
            ...
            let peer_ids = manifest
                .shares()
                .iter()
                .filter(|(_hex, share)| share.dialable())
                .map(|(_hex, share)| *share.identity())
                .collect();
...
fn verify_author(
    ...
    let author_hex = manifest.author().to_hex();

    let check_shares = previous
        .map(|p| p.shares())
        .unwrap_or_else(|| manifest.shares());

    // Presence in shares == authorized (all shares are owners)
    check_shares
        .get(&author_hex)
...
where
    ...
    L::Error: std::error::Error + Send + Sync + 'static,
{
    let our_pub_key = peer.secret().public();
...
mod tests {
    use super::*;
    use zim_crypto::{PrivateKey, SecretShare};

    fn create_test_manifest(owner: &PrivateKey) -> Manifest {
        Manifest::new(
            uuid::Uuid::new_v4(),
            "test".to_string(),
            owner,
    ...
    #[test]
    fn test_verify_author_rejects_relay_peer() {
        let owner = PrivateKey::generate();
        let relay = PrivateKey::generate();

        let mut manifest = create_test_manifest(&owner);
        manifest.add_relay(zim_core::fs::Relay::new(relay.public()));
        manifest.sign(&relay).unwrap();
