...
//!
//! Lives in `zim-api` (not `zim-did`) so the DID crate stays pure —
//! types + codec + resolver trait, no HTTP stack. This is the one
//! reqwest-backed implementation; it wraps a `reqwest::Client` (its
//! own, not [`crate::Client`] — resolution hits whatever host each
//! DID names, so there is no fixed base URL) and a small in-memory
//! cache. Each resolve:
//!
...
//! `allow_http` so a hub running on `http://127.0.0.1:8080` can be
...
// `std::time::Instant` panics on wasm32-unknown-unknown; `web-time` is a
// drop-in that maps to `Performance.now()` there and std on native.
use web_time::Instant;

use async_trait::async_trait;

use zim_did::{did_web_url, Did, DidDocument, DidMethod, DidResolver, ResolveError};
...
impl DidResolver for HttpDidResolver {
    async fn resolve(&self, did: &Did) -> Result<DidDocument, ResolveError> {
        ...
        // Cache hit? (Guard dropped before any await — std lock is fine.)
        {
