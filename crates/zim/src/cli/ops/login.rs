//! `zim login --hub <url>` — pair this daemon with a hub.
//!
...
pub struct Login {
    ...
    /// slash; we strip one if you pass one.
    #[arg(long)]
    pub hub: String,
...
impl Op for Login {
    ...
    async fn run(&self, _ctx: ()) -> Result<Self::Output, Self::Error> {
        ...
        let (hub_did, hub_nick) = if self.no_peer_add {
            ...
            let mut book =
                PeerBook::load(&home).map_err(|e| LoginError::PeerBook(e.to_string()))?;
            book.upsert(self.nick.clone(), did.clone(), Some("zim hub".to_string()));
            book.save(&home)
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match (&self.hub_nick, &self.hub_did) {
            (Some(nick), Some(did)) => {
                ...
                write!(
                    f,
                    "  next:    zim vault <name> relays add {}",
                    ui::ident(nick),
                )
            }
