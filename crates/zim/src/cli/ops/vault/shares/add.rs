//! `zim vault <target> shares add <peer>` — grant a peer access.
...
use crate::http_server::api::v0::vault::share::ShareRequest;

#[derive(Args, Debug, Clone)]
pub struct Add {
    #[arg(skip)]
    pub target: String,
    /// Peer nick (from `zim peers list`) or DID URL
    /// (`did:key:z…` / `did:web:…`). Browser-resident keys are
    /// distinguished by DID method now, not a `--non-dialable` flag.
    pub peer: String,
}
...
impl Op for Add {
    ...
    async fn run(&self, ctx: ApiContext) -> Result<Self::Output, Self::Error> {
        ...
        let r = ctx.client.call(ShareRequest { vault_id, peer }).await?;
        Ok(AddOutput {
            peer: r.peer,
            height: r.height,
        })
