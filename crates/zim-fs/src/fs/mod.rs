...
//! - **[`Fs`]**: In-memory representation of a bucket with CRUD operations
...
//! Links between nodes use [`Link`](zim_store::linked_data::Link), which includes:
//! - Hash (BLAKE3)
...
//! ## Encryption Model
//!
//! - Each node and file has its own encryption [`Secret`](zim_crypto::Secret)
//! - Secrets are stored in the parent node's [`NodeLink`]
//! - The root node's secret is shared with authorized peers via [`Share`](zim_crypto::Share)
