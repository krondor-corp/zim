...
use zim_core::iroh::DhtDiscovery;
use zim_core::iroh::Endpoint;
...
pub struct PeerBuilder<L: BucketLogProvider, State = ReadyToBuild> {
    ...
    /// the identity of the peer, as a PrivateKey
...
impl<L: BucketLogProvider, State> PeerBuilder<L, State> {
    ...
    pub fn secret_key(mut self, secret_key: PrivateKey) -> Self {
...
impl<L: BucketLogProvider> PeerBuilder<L, ReadyToBuild> {
    pub async fn build(self) -> Peer<L> {
        ...
        let secret_key = self.secret_key.unwrap_or_else(PrivateKey::generate);
        ...
        let iroh_secret = zim_core::iroh::to_iroh_secret_key(&secret_key);
        let mainline_discovery = DhtDiscovery::builder()
            .secret_key(iroh_secret.clone())
            .build()
