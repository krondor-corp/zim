...
use zim_core::fs::{AbsPath, Fs, Entry};
...
pub async fn handler(
    ...
    if query.viewer.is_none() {
        if let Some((index_path, index_mime_type)) = find_index_file(mount, path_buf).await {
            let file_data = match mount.cat(&AbsPath::from_abs(index_path.clone())).await {
                Ok(data) => data,
                Err(e) => {
    ...
    let abs_path = AbsPath::from_abs(path_buf.to_path_buf());
    let items_map = if wants_deep {
        match mount.ls_deep(&abs_path).await {
            Ok(items) => items,
            Err(e) => {
        ...
        let entries: Vec<DirectoryEntry> = items_map
            ...
            .map(|(path, node_link)| {
                ...
                let mime_type = match &node_link {
                    Entry::Dir { .. } => "inode/directory".to_string(),
                    Entry::File { .. } => node_link
                        .mime()
                        .map(|m| m.to_string())
    ...
    let items: Vec<FileDisplayInfo> = items_map
        ...
        .map(|(path, node_link)| {
            ...
            let (mime_type, is_dir) = match &node_link {
                Entry::Dir { .. } => ("inode/directory".to_string(), true),
                Entry::File { .. } => (
                    node_link
                        .mime()
                        .map(|m| m.to_string())
...
async fn find_index_file(
    ...
    for (filename, mime_type) in &candidates {
        ...
        if matches!(
            mount
                .get_entry_at_path(&AbsPath::from_abs(index_path.clone()))
                .await,
            Ok(Some(_))
        ) {
