...
use zim_crypto::{PublicKey, Secret, SecretError, SecretKey, SecretShare};
use zim_store::linked_data::{BlockEncoded, CodecError, Link};
use zim_store::{BlobsStore, BlobsStoreError};
...
#[derive(Clone)]
pub struct Fs(Arc<Mutex<FsInner>>, BlobsStore);
...
pub enum FsError {
    ...
    #[error("share error: {0}")]
    Share(#[from] zim_crypto::SecretShareError),
...
impl Fs {
    ...
    pub async fn load(
        ...
        let bucket_share = match manifest.get_share(public_key) {
            Some(share) => share,
            None => return Err(FsError::ShareNotFound),
        };
        ...
        let secret = match bucket_share.role() {
            PrincipalRole::Owner => {
                ...
                let share = bucket_share.share().ok_or(FsError::ShareNotFound)?;
            ...
            PrincipalRole::Mirror => {
                ...
                manifest
                    .public()
                    .cloned()
                    .ok_or(FsError::MirrorCannotOpen)?
            }
        };
    ...
    pub async fn load_manifest(link: &Link, blobs: &BlobsStore) -> Result<Manifest, FsError> {
    ...
    pub async fn remove_share(&self, peer_public_key: PublicKey) -> Result<(), FsError> {
        let mut inner = self.0.lock().await;
        ...
        if inner.manifest.shares_mut().remove(&key_hex).is_none() {
            return Err(FsError::ShareNotFound);
        }
    ...
    pub async fn publish(&self) -> Result<(Link, Link, u64), FsError> {
        self.save(&self.1, Some(true)).await
    }

    /// Unpublish this bucket, revoking mirror decryption access.
    pub async fn unpublish(&self) -> Result<(Link, Link, u64), FsError> {
        self.save(&self.1, Some(false)).await
    }

    pub async fn add<R>(&mut self, path: &Path, data: R) -> Result<(), FsError>
    where
        R: Read + Send + Sync + 'static + Unpin,
    {
        ...
        let link = Link::new(zim_store::linked_data::LD_RAW_CODEC, hash);
    ...
    pub async fn rm(&mut self, path: &Path) -> Result<(), FsError> {
        let path = clean_path(path);
        let parent_path = path
            .parent()
            .ok_or_else(|| FsError::Default(anyhow::anyhow!("Cannot remove root")))?;
    ...
    pub async fn mkdir(&mut self, path: &Path) -> Result<(), FsError> {
        let path = clean_path(path);
        ...
        let (parent_path, dir_name) = if let Some(parent) = path.parent() {
            ...
            return Err(FsError::Default(anyhow::anyhow!("Cannot mkdir at root")));
        };
        ...
        let parent_node = if parent_path == Path::new("") {
            ...
            match Self::_get_node_at_path(&entry, parent_path, &self.1).await {
                Ok(node) => node,
                Err(FsError::PathNotFound(_)) => Node::default(), // Will be created
        ...
        if parent_node.get_link(&dir_name).is_some() {
            return Err(FsError::PathAlreadyExists(Path::new("/").join(&path)));
        }
    ...
    pub async fn mv(&mut self, from: &Path, to: &Path) -> Result<(), FsError> {
        ...
        if to.starts_with(from) {
            return Err(FsError::MoveIntoSelf {
        ...
        if self.get(to).await.is_ok() {
            return Err(FsError::PathAlreadyExists(to.to_path_buf()));
        }
        ...
        //   d) Propagating changes up to the root (updating all ancestor nodes)
        {
            ...
            let parent_path = from_clean
                .parent()
                .ok_or_else(|| FsError::Default(anyhow::anyhow!("Cannot move root")))?;
            ...
            if parent_node.del(&file_name).is_none() {
                return Err(FsError::PathNotFound(from_clean.to_path_buf()));
            }
    ...
    pub async fn ls(&self, path: &Path) -> Result<BTreeMap<PathBuf, NodeLink>, FsError> {
        ...
        let node = if path == Path::new("") {
            ...
            match Self::_get_node_at_path(&root_node, &path, &self.1).await {
                Ok(node) => node,
                Err(FsError::LinkNotFound(_)) => {
                    return Err(FsError::PathNotNode(path.to_path_buf()))
                }
                Err(err) => return Err(err),
            }
        };
    ...
    pub async fn ls_deep(&self, path: &Path) -> Result<BTreeMap<PathBuf, NodeLink>, FsError> {
    ...
    async fn _ls_deep(
        ...
        path: &Path,
        base_path: &Path,
    ) -> Result<BTreeMap<PathBuf, NodeLink>, FsError> {
    ...
    pub async fn cat(&self, path: &Path) -> Result<Vec<u8>, FsError> {
        let path = clean_path(path);
        ...
        let link = parent_node
            .get_link(&file_name)
            .ok_or_else(|| FsError::PathNotFound(path.to_path_buf()))?;
        ...
        match link {
            ...
            NodeLink::Dir(_, _) => Err(FsError::PathNotNode(path.to_path_buf())),
        }
    }

    /// Get the NodeLink for a file at a given path
    #[allow(clippy::await_holding_lock)]
    pub async fn get(&self, path: &Path) -> Result<NodeLink, FsError> {
        let path = clean_path(path);
    ...
    async fn _get_node_at_path(
        ...
        for part in path.iter() {
            ...
            let next_link = current_node
                .get_link(&next)
                .ok_or(FsError::PathNotFound(consumed_path.clone()))?;
    ...
    pub async fn _set_node_link_at_path(
        ...
        path: &Path,
        blobs: &BlobsStore,
    ) -> Result<(NodeLink, Vec<zim_store::linked_data::Hash>), FsError> {
        let path = clean_path(path);
        ...
        for part in parent_path.iter() {
            ...
            if let Some(next_link) = next_link {
                ...
                match next_link {
                    ...
                    NodeLink::Data(..) => {
                        return Err(FsError::PathNotNode(consumed_path.clone()));
                    }
                }
                visited_nodes.push((consumed_path.clone(), node.clone()));
            } else {
    ...
    async fn _get_manifest_from_blobs(
        ...
        match blobs.stat(&hash).await {
            ...
            Ok(false) => {
                ...
                return Err(FsError::LinkNotFound(link.clone()));
            }
            Err(e) => {
                tracing::error!(
    ...
    pub async fn _get_pins_from_blobs(link: &Link, blobs: &BlobsStore) -> Result<Pins, FsError> {
    ...
    pub async fn _put_pins_in_blobs(pins: &Pins, blobs: &BlobsStore) -> Result<Link, FsError> {
    ...
    fn get_secret_for_manifest(
        ...
        match share.role() {
            PrincipalRole::Owner => {
                let secret_share = share.share().ok_or(FsError::ShareNotFound)?;
                Ok(secret_share.recover(secret_key)?)
            }
            PrincipalRole::Mirror => manifest.public().cloned().ok_or(FsError::MirrorCannotOpen),
        }
    }
    ...
    async fn apply_resolved_state(&mut self, merged_ops: &PathOpLog) -> Result<(), FsError> {
        ...
        for (path, op) in &resolved_state {
            if op.content_link.is_some() {
                ...
                match self.mkdir(&abs_path).await {
                    Ok(()) => {}
                    Err(FsError::PathAlreadyExists(_)) => {}
                    Err(e) => return Err(e),
                }
            }
        }
    ...
    pub async fn merge_from<R: super::ConflictResolver>(
        ...
        resolver: &R,
        blobs: &BlobsStore,
    ) -> Result<(MergeResult, Link), FsError> {
