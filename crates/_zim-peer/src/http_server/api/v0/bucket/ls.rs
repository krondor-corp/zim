...
pub async fn handler(
    ...
    let mount = if let Some(hash_str) = &req.at {
        let hash = hash_str
            .parse::<zim_core::linked_data::Hash>()
            .map_err(|e| LsError::InvalidHash(format!("Invalid hash format: {}", e)))?;
        let link = zim_core::linked_data::Link::new(zim_core::linked_data::LD_RAW_CODEC, hash);
        zim_core::fs::Fs::<zim_core::blobs::BlobsStore>::load(
            &link,
            state.peer().secret(),
            state.peer().blobs(),
        )
    ...
    // List items
    let items = if deep {
        mount.ls_deep(&abs_path).await?
    } else {
        mount.ls(&abs_path).await?
    };
    ...
    let path_infos = items
        .into_iter()
        .map(|(path, node_link)| {
            // Fs returns relative paths, make them absolute
