...
This is how `debug_assertions` (on for `cargo build`, off for `--release`) drives the dev/release split: `zim clean` exists only in debug builds. See [source installation](../dx/install.md#debug-vs-release-profile) for the user-facing behavior.
...
1. Create `crates/zim-peer/src/cli/ops/<group>/<name>.rs`
