...
`make dev` is just `./bin/dev` (pass extra args with `make dev ARGS="--hub"`).
...
```bash
./bin/minio up       # container `zim-minio`, API :9000, console :9001
./bin/minio down
./bin/minio status
```
...
The standalone correctness harness is `bin/fuse-e2e` — it boots a `--features fuse` daemon, mounts a vault, and cross-checks every filesystem op against the vault:

```bash
./bin/fuse-e2e          # single-node mount correctness
./bin/fuse-e2e --sync   # also the 2-node sync flow
./bin/fuse-e2e --keep   # leave the mount up for inspection
```
...
Inspect the vault log directly (note: `bin/db` is currently stale — it still references the old `node1`/`db.sqlite` layout — so query the file directly instead):
