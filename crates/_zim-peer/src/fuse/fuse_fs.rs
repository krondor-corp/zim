...
pub struct FuseFs {
    ...
    mount: Arc<RwLock<Fs<BlobsStore>>>,
    /// Fs ID
    mount_id: Uuid,
    /// Bucket ID
    bucket_id: Uuid,
...
impl FuseFs {
    ...
    /// Fetch attributes for a path via Fs
    fn fetch_attr(&self, path: &str) -> Option<CachedAttr> {
        ...
        let result = self.rt.block_on(async move {
            ...
            let entries = mount_guard
                .ls(&AbsPath::from_abs(parent_path.to_path_buf()))
                .await
                .ok()?;
                ...
                if let Some(name) = entry_path.file_name() {
                    if name.to_string_lossy() == filename {
                        ...
                        let size = if is_dir {
                            ...
                            match mount_guard.cat(&AbsPath::from_abs(abs_path.clone())).await {
    ...
    /// Fetch directory listing via Fs
    fn fetch_dir(&self, path: &str) -> Option<Vec<CachedDirEntry>> {
        ...
        tracing::debug!("FUSE fetch_dir via Fs: path={}", path);

        let result = self.rt.block_on(async move {
            let mount_guard = mount.read().await;
            match mount_guard
                .ls(&AbsPath::from_abs(fs_path.to_path_buf()))
                .await
            {
                Ok(entries_map) => {
                ...
                Err(e) => {
                    tracing::error!("FUSE fetch_dir Fs error: {:?}", e);
                    None
                }
            }
        });
    ...
    fn fetch_content(&self, path: &str) -> Option<CachedContent> {
        ...
        let result = self.rt.block_on(async move {
            let mount_guard = mount.read().await;
            match mount_guard
                .cat(&AbsPath::from_abs(fs_path.to_path_buf()))
                .await
            {
                ...
                Err(e) => {
                    tracing::error!("FUSE fetch_content Fs error: {:?}", e);
                    None
                }
            }
        });
    ...
    fn handle_truncate(&self, path: &str, size: u64, fh: Option<u64>) -> Result<(), libc::c_int> {
        ...
        // No active write buffer - need to read-modify-write via Fs
        let mount = self.mount.clone();
        let path_str = path.to_string();
        ...
        let result: Result<(), libc::c_int> = self.rt.block_on(async move {
            ...
            if size == 0 {
                ...
                let current_data = {
                    let mount_guard = mount.read().await;
                    mount_guard.cat(&abs_path).await.unwrap_or_default()
                };
                ...
                let mut mount_guard = mount.write().await;
                mount_guard
                    .add(&abs_path, std::io::Cursor::new(new_data))
                    .await
                    .map_err(|e| {
                        tracing::error!("Failed to truncate {}: {}", path_str, e);
                        libc::EIO
        ...
        let truncated_data = self.rt.block_on(async {
            ...
            mount_guard
                .cat(&AbsPath::from_abs(std::path::PathBuf::from(path)))
                .await
                .unwrap_or_default()
        });
...
impl Filesystem for FuseFs {
    ...
    fn flush(
        ...
        if let Some(data) = buffer_data {
            let mount = self.mount.clone();
            let fs_path = std::path::Path::new(&path);
            let abs_path = AbsPath::from_abs(fs_path.to_path_buf());

            let result = self.rt.block_on(async move {
                let mut mount_guard = mount.write().await;
                mount_guard.add(&abs_path, std::io::Cursor::new(data)).await
            });
    ...
    fn mkdir(
        ...
        let result = self.rt.block_on(async move {
            let mut mount_guard = mount.write().await;
            mount_guard
                .mkdir(&AbsPath::from_abs(path_buf.clone()), false)
                .await
        });
    ...
    fn unlink(&mut self, _req: &Request<'_>, parent: u64, name: &OsStr, reply: ReplyEmpty) {
        ...
        let result = self.rt.block_on(async move {
            let mut mount_guard = mount.write().await;
            mount_guard.rm(&AbsPath::from_abs(path_buf.clone())).await
        });
    ...
    fn rename(
        ...
        let mount = self.mount.clone();
        let old_abs_path = AbsPath::from_abs(std::path::PathBuf::from(&old_path));
        let new_abs_path = AbsPath::from_abs(std::path::PathBuf::from(&new_path));

        let result = self.rt.block_on(async move {
            let mut mount_guard = mount.write().await;
            mount_guard.mv(&old_abs_path, &new_abs_path).await
        });
        ...
        match result {
            ...
            Err(e) => {
                let errno = match &e {
                    ...
                    FsError::CannotMutate(_, _) => libc::EINVAL,
                    _ => libc::EIO,
                };
