...
fn default_max_import_size() -> u64 {
    zim_core::blobs::DEFAULT_MAX_IMPORT_SIZE
}
...
impl AppState {
    ...
    pub fn load_key(&self) -> Result<PrivateKey, StateError> {
        let pem = fs::read_to_string(&self.key_path)?;
        let key = PrivateKey::from_pem(&pem).map_err(|e| StateError::InvalidKey(e.to_string()))?;
        Ok(key)
    }

    /// Project this state into a runtime [`crate::ServiceConfig`].
    ///
    /// `AppConfig` is the persistent (TOML on-disk) source of truth; `ServiceConfig`
    /// is the runtime construction parameter consumed by `ServiceState::from_config`.
    /// This method is the single place where the projection happens — it eliminates
    /// hand-coded conversions drifting between the two shapes (T-007a P-3).
    ///
    /// Runtime-only fields the caller must provide:
    /// - `api_port_override` / `gateway_port_override`: CLI flags can override the
    ///   persisted defaults.
    /// - `log_level`, `log_dir`: runtime logging surface, not persisted.
    /// - `gateway_url`: external gateway URL for share/download link generation.
    pub fn build_service_config(
        &self,
        api_port_override: Option<u16>,
        gateway_port_override: Option<u16>,
        log_level: tracing::Level,
        log_dir: Option<PathBuf>,
        gateway_url: Option<String>,
    ) -> Result<crate::ServiceConfig, StateError> {
        ...
        Ok(crate::ServiceConfig {
            node_listen_addr,
            node_secret: Some(secret_key),
            blob_store: self.config.blob_store.clone(),
            jax_dir: self.jax_dir.clone(),
            max_import_size: self.config.max_import_size,
            api_port: api_port_override.unwrap_or(self.config.api_port),
            gateway_port: gateway_port_override.unwrap_or(self.config.gateway_port),
