...
pub async fn handler(
    ...
    let identity = Did::parse(&req.peer).map_err(|e| ShareError::BadPeer(e.to_string()))?;
    // *Share to a DID* — one operation across every shape. `did:key`
    // yields a single direct reach; `did:web` resolves the document
...
impl ApiRequest for ShareRequest {
    ...
    fn build_request(self, base_url: &Url, client: &Client) -> RequestBuilder {
        let url = base_url
            .join(&format!("/api/v0/vaults/{}/share", self.vault_id))
            .unwrap();
