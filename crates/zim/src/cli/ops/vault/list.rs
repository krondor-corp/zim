//! `zim vault list` — show every vault known to the daemon.
...
use crate::daemon::api::v0::vault::list::{ListRequest, VaultInfo};
...
impl fmt::Display for ListOutput {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.0.is_empty() {
            return writeln!(f, "{}", ui::dim("no vaults — `zim vault create <name>`"));
        }
