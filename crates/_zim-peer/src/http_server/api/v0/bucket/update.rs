...
pub async fn handler(
    ...
    if !mount_path_buf.is_absolute() {
        return Err(UpdateError::InvalidPath("Fs path must be absolute".into()));
    }
    let abs_mount_path = AbsPath::from_abs(mount_path_buf.clone());
    ...
    let file_exists = mount.get(&abs_mount_path).await.is_ok();
    if file_exists {
        tracing::info!("UPDATE API: Removing existing file at {}", mount_path);
        // Remove the existing file
        mount.rm(&abs_mount_path).await.map_err(|e| {
    ...
    mount.add(&abs_mount_path, reader).await?;
