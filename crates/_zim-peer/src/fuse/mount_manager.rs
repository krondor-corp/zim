//! Fs manager for FUSE filesystems
//!
...
pub struct LiveMount {
    ...
    pub mount: Arc<RwLock<Fs<BlobsStore>>>,
...
impl MountManager {
    ...
    /// from the new head.
    pub async fn on_bucket_synced(&self, bucket_id: Uuid) -> Result<(), FsError> {
        ...
        for (mount_id, live_mount) in mounts.iter() {
            if live_mount.config.bucket_id == bucket_id {
                ...
                if has_local_changes {
                    tracing::info!(
                        "Fs {} has local changes, merging with incoming sync",
                        mount_id
                    );
                    ...
                    let incoming = self
                        ...
                        .mount(bucket_id)
                        .await
                        .map_err(|e| FsError::MountLoad(e.into()))?;

                    // Merge incoming changes into local mount
                    let mut mount_guard = live_mount.mount.write().await;
                    let incoming_link = incoming.link().await;
                    match zim_core::vault::merge(&mut mount_guard, &incoming_link).await {
                        Ok(result) => {
                            ...
                            tracing::info!(
                                "Merged {} operations, {} conflicts resolved",
                ...
                tracing::debug!(
                    "Fs {} cache invalidated after bucket {} sync",
                    mount_id,
                    bucket_id
                );
            }
    ...
    pub async fn create_mount(
    ...
    ) -> Result<FuseMount, FsError> {
        // Validate bucket exists
        let bucket_info = crate::database::models::BucketInfo::get(&bucket_id, &self.db)
            .map_err(FsError::Database)?;

        if bucket_info.is_none() {
            return Err(FsError::BucketNotFound(bucket_id));
        }
        ...
        if !mount_path.exists() {
            std::fs::create_dir_all(&mount_path).map_err(|e| {
                FsError::MountPointNotFound(format!("{} (failed to create: {})", mount_point, e))
            })?;
        }

        if !mount_path.is_dir() {
            return Err(FsError::MountPointNotDirectory(mount_point.to_string()));
        }
    ...
    /// Get a mount by ID
    pub async fn get(&self, mount_id: &Uuid) -> Result<Option<FuseMount>, FsError> {
        FuseMount::get(*mount_id, &self.db).map_err(FsError::Database)
    }

    /// List all mounts
    pub async fn list(&self) -> Result<Vec<FuseMount>, FsError> {
        FuseMount::list(&self.db).map_err(FsError::Database)
    }
    ...
    pub async fn update(
        ...
        cache_size_mb: Option<u32>,
        cache_ttl_secs: Option<u32>,
    ) -> Result<Option<FuseMount>, FsError> {
        FuseMount::update(
            *mount_id,
            mount_point,
            enabled,
    ...
    pub async fn delete(&self, mount_id: &Uuid) -> Result<bool, FsError> {
        ...
        FuseMount::delete(*mount_id, &self.db).map_err(FsError::Database)
    }

    /// Start a mount (spawn FUSE process)
    pub async fn start(&self, mount_id: &Uuid) -> Result<(), FsError> {
        // Get mount config
        let mount_config = FuseMount::get(*mount_id, &self.db)
            .map_err(FsError::Database)?
            .ok_or(FsError::MountNotFound(*mount_id))?;
        ...
        // Check if already running
        {
            ...
            if let Some(live) = mounts.get(mount_id) {
                if live.session.is_some() {
                    return Err(FsError::AlreadyRunning(*mount_id));
                }
            }
        }
        ...
        // Create the FUSE filesystem with direct Fs reference for reads
        ...
        let options = {
            ...
            let bucket_name =
                crate::database::models::BucketInfo::get(&mount_config.bucket_id, &self.db)
                    .ok()
                    .flatten()
                    .map(|info| info.name)
    ...
    /// Stop a mount
    pub async fn stop(&self, mount_id: &Uuid) -> Result<(), FsError> {
        // Update status to stopping
        let _ = FuseMount::update_status(*mount_id, MountStatus::Stopping, None, &self.db);
        ...
        let mount_point = {
            ...
            if let Some(live) = mounts.get_mut(mount_id) {
                ...
                match FuseMount::get(*mount_id, &self.db) {
    ...
    pub async fn stop_all(&self) -> Result<(), FsError> {
    ...
    pub async fn start_auto(&self) -> Result<(), FsError> {
        let auto_mounts = FuseMount::auto_list(&self.db).map_err(FsError::Database)?;
    ...
    pub async fn get_live_mount(&self, mount_id: &Uuid) -> Option<Arc<RwLock<Fs<BlobsStore>>>> {
        let mounts = self.mounts.read().await;
    ...
    /// Platform-specific unmount
    async fn unmount_path(&self, mount_point: &str) -> Result<(), FsError> {
        ...
        #[cfg(target_os = "macos")]
        {
            let status = Command::new("umount")
                .arg(mount_point)
                .status()
                .map_err(|e| FsError::UnmountFailed(e.to_string()))?;
