...
pub async fn handler(
    ...
    let mount = if let Some(hash_str) = &query.at {
        match hash_str.parse::<zim_store::linked_data::Hash>() {
            Ok(hash) => {
                let link =
                    zim_store::linked_data::Link::new(zim_store::linked_data::LD_RAW_CODEC, hash);
                match zim_fs::Fs::load(&link, state.peer().secret(), state.peer().blobs()).await {
                    Ok(mount) => mount,
                    Err(e) => {
        ...
        match state.peer().logs().latest_published(bucket_id).await {
            Ok(Some((published_link, _height))) => {
                match zim_fs::Fs::load(&published_link, state.peer().secret(), state.peer().blobs())
                    .await
                {
                    Ok(mount) => mount,
                    Err(_) => {
