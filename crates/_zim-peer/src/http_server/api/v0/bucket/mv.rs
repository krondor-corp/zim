...
pub async fn handler(
    ...
    let abs_source = AbsPath::from_abs(source_path);
    let abs_dest = AbsPath::from_abs(dest_path);
    ...
    mount.mv(&abs_source, &abs_dest).await.map_err(|e| {
    ...
    fn into_response(self) -> Response {
        match self {
            MvError::InvalidPath(msg) => (
                ...
                .into_response(),
            MvError::Fs(FsError::PathNotFound(path)) => (
                http::StatusCode::NOT_FOUND,
                format!("Source not found: {}", path),
            )
                .into_response(),
            MvError::Fs(FsError::CannotMutate(path, reason)) => (
                http::StatusCode::CONFLICT,
                format!("Cannot mutate {}: {}", path, reason),
            )
                .into_response(),
            MvError::Fs(_) => (
