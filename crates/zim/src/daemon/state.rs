...
//! [`VaultHandle`](crate::daemon::api::v0::vault::extractor)
...
use zim_api::hub::HttpDidResolver;
...
use crate::daemon::accept::ContactsAcceptPolicy;
...
impl ServiceState {
    ...
    pub async fn boot(home: &Path) -> Result<Self, StateError> {
        ...
        let blobs = BlobsProvider::local(&paths::blob_index_file(home), &blobs_path)
            .await
