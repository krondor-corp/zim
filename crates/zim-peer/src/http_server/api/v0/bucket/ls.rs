...
pub async fn handler(
    ...
    let mount = if let Some(hash_str) = &req.at {
        let hash = hash_str
            .parse::<zim_store::linked_data::Hash>()
            .map_err(|e| LsError::InvalidHash(format!("Invalid hash format: {}", e)))?;
        let link = zim_store::linked_data::Link::new(zim_store::linked_data::LD_RAW_CODEC, hash);
        zim_fs::Fs::load(&link, state.peer().secret(), state.peer().blobs()).await?
    } else {
