...
Default URL: `http://localhost:8080/`. The embedded peer keeps its SQLite database and blob store under `./data/zim-hub/` (created on first launch).

`make hub` watches `crates/zim-hub/{src,templates,static,Cargo.toml}` and auto-reloads via `cargo-watch`:
...
|----------|---------|--------------|
| `ZIM_HUB_LISTEN` | `127.0.0.1:8080` | HTTP listen address. |
...
| `HUB_PORT` | `8080` | Convenience override for `ZIM_HUB_LISTEN`'s port via the Makefile (`HUB_PORT=8090 make hub`). |
...
The daemon's HTTP API listens on `http://localhost:3000` and its local Web UI on `http://localhost:8080`. If you also run the hub, point one of them at a different port (`ZIM_HUB_LISTEN=127.0.0.1:8090 make hub`).
...
`./bin/cleanup --all` also removes `./data/` (dev node state) and `.minio` (MinIO local). `./bin/cleanup --dry-run` shows what would be removed without touching anything.
