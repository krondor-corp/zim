...
use crate::database::models::BucketStatus;
...
pub async fn handler(
    ...
    let mount = Fs::init(id, req.name.clone(), owner, blobs)
        .await
        .map_err(|e| {
    ...
    tracing::info!(
        "CREATE BUCKET: Fs initialized successfully for bucket {}",
        id
    );
    ...
    BucketStatus::set(&id, BucketStatus::Active, None, state.database()).map_err(|e| {
        tracing::error!(
