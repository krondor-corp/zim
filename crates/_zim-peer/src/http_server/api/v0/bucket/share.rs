...
pub enum ShareRole {
    ...
    #[default]
    Owner,
    /// Relay - can sync published-set blobs but has no share
    Relay,
}
...
pub async fn handler(
    ...
    match req.role {
        ShareRole::Owner => {
            mount.add_share(peer_public_key, true).await?;
            tracing::info!(
                "SHARE API: Fs.add_share(dialable=true) completed for peer {}",
                req.peer_public_key
            );
        }
        ShareRole::Relay => {
            mount
                .add_relay(zim_core::fs::Relay::new(peer_public_key))
                .await;
            tracing::info!(
                "SHARE API: Fs.add_relay() completed for peer {}",
                req.peer_public_key
            );
        }
    }
