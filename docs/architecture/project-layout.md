...
### `crates/zim-peer` - CLI & Daemon (lib + bin)
...
### `crates/zim-core` - Core Library

Shared library (`zim-core`) with crypto, storage, and peer protocol.
...
SQLite + object storage backend for blob data (`zim-core`).
