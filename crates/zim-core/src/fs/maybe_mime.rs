//! A serializable wrapper around `Option<mime::Mime>`.
//!
//! `mime::Mime` doesn't impl `Serialize`/`Deserialize` and isn't
//! `Option`-friendly out of the box. [`MaybeMime`] serializes as a
//! plain string (or `null`) so it sits cleanly inside DAG-CBOR
//! [`Entry::File`](super::Entry::File) records.
...
/// An optional MIME type with custom serde so it round-trips through
/// CBOR / JSON as a string (or `null`). Construct via the public field
/// or via [`Self::from_path`].
#[derive(Debug, Clone, PartialEq)]
pub struct MaybeMime(pub Option<Mime>);

impl MaybeMime {
    /// Infer a MIME type from `path`'s file-extension via `mime_guess`.
    /// Returns `MaybeMime(None)` when no guess is available or the
    /// guessed string fails to parse.
