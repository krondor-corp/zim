use zim_crypto::{PublicKey, SecretKey};
...
impl<L: BucketLogProvider> Peer<L> {
        ...
        L::Error: std::error::Error + Send + Sync + 'static,
    {
        ...
        let manifest: zim_fs::Manifest = self
            .blobs()
        ...
        let our_key = zim_crypto::PublicKey::from(*self.secret().public()).to_hex();
    ...
    pub async fn mount(&self, bucket_id: Uuid) -> Result<Fs, FsError> {
        // Get current head link from log
        let (link, _height) = self
            .log_provider
            .head(bucket_id, None)
            .await
            .map_err(|e| FsError::Default(anyhow!("Failed to get current head: {}", e)))?;
    ...
    /// - Failed to load mount from blobs
    pub async fn mount_for_read(&self, bucket_id: Uuid) -> Result<Fs, FsError> {
        use zim_fs::PrincipalRole;
        ...
        let our_role = Fs::load_manifest(&head_link, &self.blobs_store)
            .await
            .ok()
            .and_then(|m| {
        ...
        match our_role {
            ...
            _ => {
                ...
                let (link, _) = self
                    ...
                    .await
                    .map_err(|e| {
                        FsError::Default(anyhow!("Failed to get latest published: {}", e))
                    })?
                    .ok_or_else(|| FsError::Default(anyhow!("No published version available")))?;
                Fs::load(&link, &self.secret_key, &self.blobs_store).await
            }
        }
    }
    ...
    pub async fn save_mount(&self, mount: &Fs, publish: Option<bool>) -> Result<Link, FsError>
    where
        L::Error: std::error::Error + Send + Sync + 'static,
    {
        ...
        self.log_provider
            ...
            .await
            .map_err(|e| FsError::Default(anyhow!("Failed to append to log: {}", e)))?;
