...
impl MountManager {
    ...
    pub async fn list(&self) -> Result<Vec<FuseMount>, FsError> {
        FuseMount::list(&self.db).await.map_err(FsError::Database)
    }
    ...
    pub async fn get_live_mount(&self, mount_id: &Uuid) -> Option<Arc<RwLock<Fs>>> {
        let mounts = self.mounts.read().await;
