...
use uuid::Uuid;
use zim_core::blobs::{BlobStore, BlobsStore};

use zim_core::fs::{AbsPath, Fs};
use zim_core::fs::{FsError, Entry};
...
pub struct ExportResponse {
    ...
    pub link: zim_core::linked_data::Link,
    pub height: u64,
...
pub async fn handler(
    ...
    // Load the mount
    let mount = Fs::load(&head_link, secret_key, blobs)
        .await
        .map_err(ExportError::Fs)?;

    // Get the bucket name from the mount's manifest
    let fs_inner = mount.inner().await;
    let bucket_name = fs_inner.manifest().name().to_string();
...
async fn export_mount_to_filesystem(
    ...
    let items = mount
        .ls_deep(&AbsPath::from_abs(PathBuf::from("/")))
        .await
        .map_err(ExportError::Fs)?;

    for (path, leaf) in items {
        match leaf {
            Entry::File { link, secret, .. } => {
