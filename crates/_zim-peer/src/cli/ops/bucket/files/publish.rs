...
use zim_peer::http_server::api::v0::bucket::files_publish::{
    FilesPublishRequest, FilesPublishResponse,
};
...
pub struct Publish {
    ...
    /// Path of the file inside the bucket to publish
    pub path: PathBuf,
...
pub struct PublishOutput {
    pub response: FilesPublishResponse,
}

impl fmt::Display for PublishOutput {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let r = &self.response;
        writeln!(f, "{}", ui::success("Published", &r.display_path))?;
        write!(
            f,
            "{}",
...
impl crate::cli::op::Op for Publish {
    ...
    async fn execute(&self, ctx: &crate::cli::op::OpContext) -> Result<Self::Output, Self::Error> {
        ...
        let request = FilesPublishRequest {
            bucket_id,
            path: self.path.clone(),
            display_path: self.display_path.clone(),
        };
        let response: FilesPublishResponse = client.call(request).await?;
